...
public class Board {
    ...
    public static boolean isPosEnemyPawn(Position pos, String side) {
        ...
        if (cp == null) {
            return false;
        } else if (!(cp.getSide().equals(side)) && cp instanceof Pawn) {
            return true;
        }
        return false;
    }
    ...
    public static boolean isPosFriendlyRook(Position pos, String side) {
        ...
        if (cp == null) {
            return false;
        } else if (cp.getSide().equals(side) && (cp instanceof Rook)) {
            return true;
        }
        return false;
    }
    ...
    public static boolean isEnPassantable(Position pos) {
        ...
        if (target.equals(lastMovedPiece)) {
            ...
            if (pos.getUp().getUp().equals(srcLastMove)) {
                return true;
            }
        }
        return false;
    ...
    // Checkmate == No valid moves + Checked
    // Stalemate == No valid moves + Not in check
    // Note: Modifying array while iterating through it is a no-go!
    public boolean hasValidMoves() { // current turn
        if (debugger) { System.out.println(">>> HVM: Checking if valid moves available"); }
            ...
            if (cp.getSide().equals(turn)) {
                for (Position pos : cp.getMovement(pieces)) {
                    if (isValidMove(cp, pos)) {
                        if (debugger) { System.out.println(">>> HVM: Checked: true"); }
                        return true;
                    }
                }
            }
        }
        if (debugger) { System.out.println(">>> HVM: Checked: false"); }
        return false;
    }
    ...
    private ChessPiece promote(ChessPiece piece, String side) {
        Position pos = piece.getPos();
        removePiece(piece);
        Queen promoted = new Queen(side, pos);
        pieces.add(promoted);
        return promoted;
    }
    ...
    // Returns ChessPiece to account for piece change, e.g. promotion
    public ChessPiece movePiece(ChessPiece piece, Position dsc) {
        ...
        // Check if pawn promoting
        if (piece instanceof Pawn) {
            System.out.println("HI");
            // Promote white pawn
            if (dsc.getX() == Position.getTopBound()) {
                System.out.println("HI2");
                piece = promote(piece, piece.getSide());
            }
            // Promote black pawn
            else if (dsc.getX() == Position.getBottomBound()) {
                piece = promote(piece, piece.getSide());
            }
        }
